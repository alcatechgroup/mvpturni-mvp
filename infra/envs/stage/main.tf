...
module "cloud_run_api" {
  ...
  env_vars = {
    ...
    # Referência direta à saída do módulo admin (api → admin, acíclico). Antes era um
    # run.app hardcoded com o hash do turni-mvp — quebraria ao recriar em turni-homol.
    BACKOFFICE_URL = module.cloud_run_admin.service_url
...
resource "google_cloud_run_v2_service" "pagarme_mock" {
  ...
  template {
    ...
    containers {
      ...
      env {
        # URL pública do Cloud Run api. Referenciar module.cloud_run_api aqui criaria ciclo
        # (o api referencia a uri deste serviço em PAGARME_BASE_URL), por isso via var de
        # DUAS FASES: vazio no 1º apply; após o api subir, preencher api_public_url com
        # `terraform output api_url` e reaplicar. Necessário porque o hash do run.app muda
        # ao recriar o homolog no projeto turni-homol (não dá para hardcodar como antes).
        name  = "PAGARME_WEBHOOK_TARGET"
        value = var.api_public_url != "" ? "${var.api_public_url}/api/webhooks/pagarme" : "https://api-pending.invalid/api/webhooks/pagarme"
      }
      env {
