...
blocked_by_sprint: SPRINT-2026-W27.5  # ADR-018 accepted exige refator UUID antes de o EPIC-003 começar (STORY-056 usa external_reference apontando para IDs do Turni — janela cirúrgica para virar o tipo se fecha nesse commit)
pivoted_by: PDR-017  # 2026-06-04 — Pagar.me real adiado para próxima wave; MVP usa fake genérico atrás da mesma ACL; banner global em homolog (STORY-075); STORY-056-B abandonada; ADR-005 + ADR-016 precisam de revisão pelo Arquiteto
closure_rule: "Fechamento por goal-atingido: encerra quando STORY-055..067 + STORY-073 + STORY-075 estiverem `done` E STORY-068 (validador) tiver emitido veredito em `validation/report.md` aceitável pelo PO (`approved` ou `approved_with_pending` que o PO assuma como goal-atingido). Soft-cap em 2026-07-04 (~31 dias corridos) serve como gatilho de reavaliação — não é prazo de entrega. Se até 2026-06-20 (D+16) o caminho até `ativo` não estiver vivo em homolog, abrir mini-sprint W29 dedicada ao check-out + captura + Pix em vez de inflar a W28. STORY-073 (fix `schedule:run`) é ortogonal e pode iniciar imediatamente — quita F-NB-1 do EPIC-002 carregada da W27. STORY-075 (banner global PDR-017) também ortogonal — frontend, S. STORY-056-B (contract test sandbox) ABANDONADA por PDR-017."
goal: "Ciclo do turno vivo em homolog (EPIC-003) com pagamento via fake genérico (PDR-017): contratante aceita candidatura no Backoffice → fake pré-autoriza `total_contratante` via ACL `preAutorizar` → turno em `confirmado` → profissional gera PIN de check-in (4 dígitos) com flag de geofencing capturada → contratante valida o PIN → turno transita para `ativo` com cronômetro bilateral vivo em tempo real nos dois lados (latência ≤ 2s) → profissional gera PIN de check-out → contratante valida → turno transita para `finalizado` → fake captura via ACL `capturar` → Pix simulado confirma dentro do SLA configurado (default ~30s; promessa pública 'Pix em 15 min' demonstrada como simulação). Banner global em homolog 'Ambiente de teste — pagamentos simulados' (STORY-075). Habitualidade (PDR-002) aplicada nos 4 cenários no momento do aceite (PF bloqueia 3ª, PJ alerta+override registrado no AceiteEletronico). Geofencing (PDR-008) alerta-e-registra sem bloquear. Cancelamento antes do check-in libera a pré-autorização via ACL `liberar`; `no_show_pro` por timeout libera também (PDR-007 mínimo, sem motor de penalidade). 3 ADRs novas aceitas (ADR-015 modelo Turno + AceiteEletronico, ADR-016 ACL de pagamento + fake genérico + idempotência + webhook interno [revisada pós-PDR-017], ADR-017 tempo real cronômetro + geolocalização). Validador independente (STORY-068, 4ª aparição após STORY-011/025/054) emite veredito do EPIC-003. **Integração Pagar.me real adiada para a próxima wave atrás da mesma ACL** — ACL é o ativo arquitetural durável que esta sprint preserva."
---
...
Esta sprint **abre o épico mais pesado e mais arriscado da WAVE-2026-01** — EPIC-003 (Aceite, PIN bilateral e Pix). É o **coração da promessa pública do Turni**: dois dos três pilares do produto (PIN Bilateral e Pix em 15 min) materializam aqui pela primeira vez. Sem isso, o produto vira "publicar vaga + candidatar-se em app". A WAVE-2026-01 só fecha sua hipótese central depois desta sprint.

**Pivô PDR-017 (2026-06-04):** integração Pagar.me real adiada para a próxima wave; MVP usa **fake genérico** atrás da mesma ACL desenhada em ADR-005 (a ACL é o ativo arquitetural durável). Promessa pública "Pix em 15 min" mantida como simulação. Banner global em homolog (STORY-075). Sem dependência de credenciais externas — risco técnico nº 1 da onda neutralizado.
...
1. **Spike antes de implementação** (lição W27 — STORY-048 L cabe em 1 sessão quando o spike precede de verdade). 3 spikes do Arquiteto na base (modelo Turno + máquina de estados, ACL de pagamento + fake, tempo real + geolocalização) destravam as 10 estórias de implementação.
...
- **Escopo**: **16 estórias** — EPIC-003 inteiro (14 originais + STORY-075 banner global por PDR-017) + STORY-073 (bugfix carry-forward do EPIC-002 — quita F-NB-1 da STORY-054). Mix: **2 S + 12 M + 2 L**. STORY-056-B (contract test sandbox) **abandonada em 2026-06-04 por PDR-017** — não conta no sizing.
  - As **duas L** (STORY-056 ACL+fake e STORY-063 cronômetro bilateral) são candidatas naturais a estouro de sessão única. Gatilho de quebra documentado na própria estória; agente escala ao PO **antes** de inflar. Critérios de quebra:
    - STORY-056 → separar `adapter + fake em container` (back-end) de `contract test contra sandbox real` (CI/IaC) em duas estórias. **Aplicado em 2026-06-04 — STORY-056-A entregue, STORY-056-B abandonada por PDR-017.**
...
| ID        | Título                                                                                                             | Épico    | Tipo           | Papel                    | Tamanho | Status atual (2026-06-07) |
| --------- | ------------------------------------------------------------------------------------------------------------------ | -------- | -------------- | ------------------------ | ------- | ------------ |
| STORY-055 | Spike Arquiteto — modelo Turno + AceiteEletronico imutável + máquina de estados                                    | EPIC-003 | spike          | arquiteto                | M       | **done** (2026-06-04, ADR-015) |
| STORY-056 | Spike Arquiteto — ACL de pagamento (provider-agnóstico) + fake genérico + idempotência + webhook interno (PDR-017) | EPIC-003 | spike          | arquiteto                | **L**   | **done** (2026-06-04, ADR-016) |
| STORY-057 | Spike Arquiteto — tempo real cronômetro bilateral + geolocalização Haversine                                       | EPIC-003 | spike          | arquiteto                | M       | **done** (2026-06-04, ADR-017) |
| STORY-058 | Aceitar candidatura no WebApp + AceiteEletronico imutável + pré-autorização via gateway (fake — PDR-017)           | EPIC-003 | implementation | programador              | M       | **done** (2026-06-05, rc.70) |
| STORY-059 | Lista "Meus turnos" (profissional) + "Vagas confirmadas" (contratante) no WebApp                                   | EPIC-003 | implementation | programador (+ designer) | S       | **done** (2026-06-05, rc.71) |
| STORY-060 | Detalhe do turno (ambos os lados) + timeline + trilha de auditoria visível                                         | EPIC-003 | implementation | programador (+ designer) | M       | **done** (2026-06-05, rc.72) |
| STORY-061 | PIN de check-in — geração pelo profissional + captura de geofencing (PDR-008)                                      | EPIC-003 | implementation | programador (+ designer) | M       | **done** (2026-06-05) |
| STORY-062 | Validação do PIN de check-in pelo contratante + transição para `ativo`                                             | EPIC-003 | implementation | programador (+ designer) | M       | **done** (2026-06-05) |
| STORY-063 | Cronômetro bilateral vivo em tempo real (latência ≤ 2s)                                                            | EPIC-003 | implementation | programador (+ designer) | **L**   | **done** (2026-06-05) |
| STORY-064 | PIN de check-out — geração + validação + transição para `finalizado`                                               | EPIC-003 | implementation | programador (+ designer) | M       | **done** (2026-06-06) |
...
| STORY-067 | Notificações in-app + e-mail dos eventos do turno (8 templates via STORY-020)                                      | EPIC-003 | implementation | programador (+ designer) | M       | **in_review** (rc.83 — aguarda PO) |
...
STORY-058 (aceitar candidatura + pré-autorização Pagar.me sandbox)
    ...
    └─► STORY-073 (fix schedule:run) — ortogonal total, sem dependência (carry-forward EPIC-002, pode iniciar a qualquer momento)
    └─► STORY-075 (banner global homolog PDR-017) — ortogonal total, frontend, sem dependência
                                                    │
                                                    ▼
...
Esta sprint entrega o **ciclo do turno ponta a ponta** em homolog com **pagamento via fake genérico (PDR-017)** atrás de ACL provider-agnóstica — primeira vez que a promessa pública do Turni vira observável (como simulação; integração real Pagar.me na próxima wave).
...
- Profissional `ativo` consegue:
  ...
  - Em `finalizado`, ver "Pix enviado" com confirmação do gateway (fake — PDR-017; banner global em homolog deixa explícito que é simulação).
...
- Equipe Turni aceita candidatura no painel de candidatos (STORY-051) com 1 clique → turno criado em `confirmado` → AceiteEletronico imutável anexado (cláusula override PJ se 3ª alocação semanal — PDR-002) → pré-autorização disparada via ACL de pagamento (fake — PDR-017) de forma idempotente.
...
- **100%** das tentativas no caminho feliz completam o ciclo `confirmado → finalizado → Pix enviado` (fake em modo `success` — PDR-017 troca métrica probabilística de sandbox por simulação determinística).
- Promessa "Pix em ≤ 15 min" demonstrada com fake configurado para esse SLA: 100% dos turnos confirmam dentro da janela. Default em testes é ~30s.
...
- **ADR-016** — Implementação concreta da ACL de pagamento decidida em ADR-005, **revisada pós-PDR-017** pelo Arquiteto: ACL provider-agnóstica + fake genérico (driver único no MVP) + idempotência (chave + tabela `pagamento_operacoes`) + webhook interno emitido pelo fake com HMAC validado. **Contract test contra sandbox real removido** (STORY-056-B abandonada).
...
- **ADRs vigentes** (todas aceitas em EPIC-000/EPIC-001/EPIC-002 + EPIC-010): ADR-000 (Postgres), ADR-001/002/003 (stack), ADR-004 (GCP), ADR-005 (Pagar.me alto nível — **a ser revisada pelo Arquiteto pós-PDR-017**: desenho da ACL continua válido, escolha de PSP no MVP muda), ADR-006 (habitualidade — STORY-058 consome), ADR-007/008 (Sanctum + Argon2id + log JSON + health), ADR-009 (modelo identidade), ADR-010 (template imutável — espelhado em AceiteEletronico do turno), ADR-011 (provedor e-mail — STORY-067 consome), ADR-012 (landing — não afeta), ADR-013 (modelo Vaga + Candidatura — STORY-058 consome), ADR-014 (algoritmo Match — não afeta esta sprint), ADR-016 (ACL de pagamento — `proposed` via STORY-056; **a ser revisada pelo Arquiteto pós-PDR-017** para refletir "ACL provider-agnóstica + fake genérico" em vez de "ACL Pagar.me sandbox"), **ADR-018 (UUIDv7 em PKs — aplicado pela SPRINT-2026-W27.5 antes desta sprint ativar; todas as estórias do EPIC-003 citam ADR-018; modelo Turno + AceiteEletronicoTurno + pagamento_operacoes têm `id` uuid; `external_reference` carrega UUID string; URLs RESTful aceitam UUID; DTOs Flutter tipam `id` como `String`)**.
- **PDRs vigentes (com destaque para o pivô desta sprint):** PDR-001 a PDR-016 conforme já vinha — e **PDR-017 (Pagamento via fake genérico no MVP; Pagar.me adiado para a próxima wave; ACL preservada; banner global em homolog; promessa "Pix em 15 min" mantida como simulação)**.
...
|---|---|---|---|---|
| ~~**Pagar.me sandbox é o risco técnico nº 1 da onda**~~ | ~~alta~~ | ~~alto~~ | **NEUTRALIZADO por PDR-017 (2026-06-04)** — Pagar.me sai do MVP; fake genérico não tem variabilidade externa. Risco novo abaixo: divergência futura entre fake e Pagar.me real quando trocar adapter na próxima wave. | — |
| **Fake genérico diverge do Pagar.me real quando o adapter for trocado na próxima wave** | média | alto (próxima wave) | Fake emite payloads em formato Pagar.me-compatível (mesmo contrato versionado em `docs/project-state/integrations/pagarme/contract.md`); épico dedicado da próxima wave terá contract test contra sandbox real; surpresa esperada e localizada no novo adapter. | Arquiteto + PO (próxima wave) |
...
| ~~STORY-056 (Pagar.me, L) estoura sessão única~~ | — | — | **MITIGADO em 2026-06-04** — quebra aplicada: STORY-056-A entregou ACL+fake+idempotência+webhook (in_review); STORY-056-B (contract test sandbox) abandonada por PDR-017. | — |
...
| ~~Pix sandbox demora > 15 min em janela de pico do Pagar.me~~ | — | — | **NEUTRALIZADO por PDR-017** — Pix é simulado pelo fake; SLA configurável (default ~30s; promessa "15 min" demonstrada com fake nessa janela, 100% confirma). | — |
| ~~Webhook entrante do Pagar.me em ambiente local não dispara~~ | — | — | **NEUTRALIZADO** — STORY-056-A já entregou o fake emitindo webhook assinado HMAC verificado em container real; `make setup` continua 100% offline (princípio #6). | — |
...
| ~~Pagar.me sandbox tem janela de manutenção não anunciada~~ | — | — | **NEUTRALIZADO por PDR-017** — sandbox sai do MVP. | — |
| **Banner global em homolog (STORY-075) é esquecido em alguma tela autenticada** — usuário externo demonstrativo toma pagamento simulado como real | média | médio | CA-1 da STORY-075 cobre "toda tela autenticada"; validador verifica visualmente em homolog antes de fechar STORY-068; tornar o teste do banner um requisito de smoke do auto-update da STORY-037 evitaria regressão silenciosa (deixar como follow-up). | PO + Validador |
...
7. **Snapshot de payload explicável nasce na estória que cria o dado** (aprendizado W27 STORY-051) — STORY-058 emite AceiteEletronico com payload renderizado completo e imutável; STORY-061 emite evento de check-in com geofencing completo (não só flag); STORY-065 emite evento de captura com `charge_id` retornado pelo gateway (fake — PDR-017; formato Pagar.me-compatível) e timestamp do Pix.
