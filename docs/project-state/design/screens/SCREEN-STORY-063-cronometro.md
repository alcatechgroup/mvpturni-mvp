...
| 2026-06-05 | STORY-064 estende o componente (mudança consciente, aprovada por Alexandro) | claude-opus-4-8 (programador) | (a) linha do congelado trocada de "Aguardando check-out — duração final:" para **"Aguardando validação — duração:"** (CA-6 da 064 — "duração final" mentia: recusa/cancelamento retomam o tempo); (b) estado **`finalizado`** adicionado ("Turno finalizado — duração: {HH:MM:SS}", terminal, sem polling); (c) premissa do §10 cumprida: `encerrado_em` bilateral em `aguardando_checkout` vem do evento `checkout_solicitado` que a 064 grava; ver SCREEN-064 §4.10/4.11 |
