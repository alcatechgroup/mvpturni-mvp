...
- **[BUG homolog 413 — corrigido, rc.21]** Foto de celular (~1,3 MB) era barrada pelo **nginx** com **413** (`client_max_body_size` default 1 MB) antes de chegar no Laravel → app mostrava "Não conseguimos enviar agora". O E2E não pegou porque o fixture é um PNG 1×1 e o dev server é `php -S` (sem nginx). **Fix:** `nginx.conf client_max_body_size 8m` + `Dockerfile.prod upload_max_filesize=8M/post_max_size=10M` (≥ limite de 5 MB da app). **Guardas:** teste de feature (foto ~2 MB → 201) + **smoke pós-deploy** no `release.yml` (upload >1 MB não pode voltar 413). Verificado na imagem de prod local (1,3 MB→422, 9 MB→413) e ao vivo em homolog (cadastro completo → 201). CSRF descartado (POST /api/login = 200).
