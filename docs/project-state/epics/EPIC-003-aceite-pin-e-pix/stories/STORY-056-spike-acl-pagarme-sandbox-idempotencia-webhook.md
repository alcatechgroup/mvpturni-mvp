...
- **(2026-06-04, verificação do `make setup`)** O fake era **stateless**: `capture`/`cancel` emitiam webhook sem `external_reference` (o provedor real é stateful e ecoa a referência da charge). Como `ProcessarWebhookPagarmeJob` só emite o evento de domínio quando `turnoId !== null`, `CapturaConfirmada`/`PreAutorizacaoLiberada` **nunca seriam emitidos** via fake — o webhook era marcado `processado_em` sem evento (falha silenciosa; os testes não pegaram porque o E2E usa `Http::fake` com payloads completos, não o container). Fix: estado mínimo `charge_id → {external_reference, amount}` em arquivo com `flock` no fake; captura total agora também ecoa o `amount` original (conforme `contract.md`). Gotcha de PHP no fix: `const` global só existe quando a execução passa pela linha — declarar no topo, não junto das funções no rodapé.
