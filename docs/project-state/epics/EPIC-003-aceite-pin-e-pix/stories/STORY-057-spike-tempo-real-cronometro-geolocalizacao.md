...
- **Evidência parcial capturada (2026-06-04):** cronômetro sincronizado e rodando em **Android** (07:52:32) e **iPhone** (07:53:25) — diferença só por timing das fotos; ambos ancorados no mesmo `iniciado_em`. **Android** capturou geolocalização e o backend calculou `Fora do raio: 71798.3 m` (Haversine, PDR-008 alerta-e-registra). **iPhone** (Safari/PWA standalone) não dava feedback na captura → corrigido na rc.66 (guarda de timeout + feedback de todos os desfechos; `enableHighAccuracy:false`+`maximumAge` p/ iOS).
- **Nota:** "Tempo decorrido" é uma **duração** (sem fuso); o valor alto reflete o tempo real desde o `check_in_at` semeado (~8h atrás). Reseed com check-in recente disponível se quiser timer "do zero".
- **Diagnóstico do "erro de envio" no iPhone (rc.67):** não era bug de iOS — era **RBAC correto**. O iPhone estava logado como **contratante**, e `POST /checkin-geo` é só do **profissional** (404 → erro). Geofencing é a captura do profissional no check-in (PDR-008); o contratante valida o PIN. A rc.67 expõe `sou_profissional` no cronômetro e a tela só oferece "Capturar localização" ao profissional (Android já validou: `Fora do raio: 71798.3 m`); ao contratante explica o papel. A PoC agora demonstra também a separação de papéis do PDR-008.
- **CA-5 demonstrada:** cronômetro sincronizado ≤ 2s em 2 dispositivos (Android + iPhone) + geofencing calculado no backend via Haversine (lado profissional). Falta só anexar o screenshot final (opcional).
