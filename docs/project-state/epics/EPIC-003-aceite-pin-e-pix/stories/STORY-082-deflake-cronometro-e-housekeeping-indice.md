...
- **`checkout` totalmente DEFLAKADO (3 flakes sanados).** Além da pendura (`pumpAndSettle`+timer → `_assenta`) e da colisão de Hero do SnackBar (→ `_semSnackBar`), o 3º flake — "PIN inválido" intermitente — foi RESOLVIDO após achar a causa-raiz via **audit do Postgres**: o teste selecionava o card por **rótulo de estado** com `.first`, e o par `*.checkout.seed` acumula turnos **leftover** (o seeder `seedTurnoNaJanela` cria um novo `confirmado` quando um run interrompido deixa o anterior em `ativo`/`aguardando_checkout`; provei: o `checkout_solicitado` do run caiu num turno e o check-in noutro). Com vários turnos no mesmo estado, a **fase 2 gerava o PIN num turno e a fase 3 validava em outro**. Fix: capturar o `turnoId` da rota na fase 1 e fixar TODAS as fases nele (`_cardComId` → `turno-card-{id}`), imune a leftovers. **Estabilidade: 7/7** (1 + 6/6) com a pilha de leftovers presente — antes ~1/4. NÃO era PIN efêmero nem regressão de feature (minha 1ª hipótese estava errada; a leitura do código não bastou, o audit fechou). `checkout` segue **FORA do gate** (decisão do PO por custo ~2-3 min) mas agora **pronto para reativar** (descomentar no `turnos_test.dart`).
...
- Checkout deflakado: causa-raiz do "PIN inválido" provada por audit do Postgres (`checkout_solicitado` num turno × check-in noutro, pilha de leftovers do par seed); estabilidade `/tmp/checkout-estab.log` (6/6) + 1 run com leftovers presentes.
