...
- **Flake do guard de índice (`HabitualidadeIndexTest`, STORY-055) consertado:** com poucas linhas em `turnos`, o Index Only do composto paga heap-fetch (visibility map frio dentro da transação do RefreshDatabase) e o Bitmap no índice parcial `(profissional_id, status)` empata/vence conforme o que a suíte rodou antes — ao crescer a suíte de turnos (esta estória), o desempate virou e o teste quebrou o pre-push. Fix determinístico: 20 turnos-lastro do mesmo profissional em OUTRO estabelecimento tornam o composto estritamente mais barato em qualquer ordem de suíte (ANALYZE sozinho não bastou).
- **Vagas FECHADAS exibem "Ver candidatos"** (`_podeVerCandidatos` da 047) e o filtro "Ativas" inclui fechadas com data futura — qualquer finder `.first` por texto em "Minhas vagas" precisa ancorar também na contagem de pendentes (helper `aprovacao_helpers.dart`).
...
- CA-9 → E2E backend: 4 cenários PDR-002 em `AprovarCandidaturaTest`+`HabitualidadeAceiteTest` (PF 1ª/2ª libera; PF 3ª bloqueia sem turno; PJ 3ª override com cláusula; virada de semana reseta) contra Postgres real. UI em **browser real** (`integration_test` same-origin, IDR-021, gate local IDR-004): `aprovar_candidatura_test` (caminho feliz ponta a ponta), `aprovar_bloqueio_pf_test` (PF 3ª → D2, nada consumido) e `aprovar_override_pj_test` (PJ 3ª → pré-aviso D1 → D3 → override → turno). A virada de semana é regra de banco — sem UI própria, fica no backend. Cenários seedados pelo `AprovacaoCandidaturaSeeder` (3 funções exclusivas, semanas isoladas em 2027, rotação por ciclo consumido).
...
- Deploy de homologação: rc.69 (2026-06-05, caminho feliz) e **rc.70** (2026-06-05, seeder com os **3 cenários** de aprovação — feliz/Camareira, bloqueio PF/Copeiro, override PJ/Hostess — + E2E browser real dos 3 ramos). **Verificação do Alexandro pendente** (roteiro em chat: turno + `pagamento_operacoes` `concluida` + audit `pagamento.pre_autorizado`; D2 sem efeitos; D3 com `habitualidade_override=true` + cláusula 10).
