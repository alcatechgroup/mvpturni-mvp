...
- [x] `index.json`: `status: done` (após CI verde + smoke homolog). **Feito 2026-06-11** — CI verde (run 27372520746), Deploy Stage verde (run 27372780215), smoke `/disputas` em stage OK (HTTP fail-secure + visual logado chancelado pelo PO).
...
- **ATUALIZAÇÃO 2026-06-11 (fechamento da estória, PO mandou destravar via fix):** o Trivy vermelho NÃO era CVE base não-fixável. Reprodução local mostrou 4 CVEs **HIGH fixáveis** em pacotes OS da base `alpine:3.22.4` (openssl/`libcrypto3`/`libssl3` CVE-2026-45447 → 3.5.7-r0; `libxml2` CVE-2026-6732 → r1; `nginx` CVE-2026-49975 → r3) — versões corrigidas já publicadas no repo 3.22. O `apk add` não atualiza pacotes pré-instalados na base; **fix = `apk upgrade --no-cache` no estágio runtime de `infra/docker/{api,admin}/Dockerfile.prod`** (commit `019b668`). Re-scan Trivy local pós-fix: **0 CRITICAL/HIGH** em ambas. **CI run `27372520746` 100% verde na main.** O `Commit lint` (header de 162 chars no commit `6c367f1`) curou-se no commit seguinte (≤100). BUG-001 → resolvido (fix_link `019b668`).
- Deploy homolog: **Deploy Stage manual** (mesmo padrão da STORY-094) — disparado pelo PO via agente: **run `27372780215` 100% verde** (build+push api/admin/pagarme-mock → stage, migrate, deploy Cloud Run + Firebase). Smoke `/disputas`: serviço no ar (`version: stage-main` em `turni-admin-stage…run.app` / `admin.stage.turni.com.br`); **HTTP fail-secure confirmado** (`/disputas` anônimo → 302 `/login`, evidência de CA-5); **smoke visual logado chancelado pelo PO** em 2026-06-11. Obs.: o env é `stage` (turni-stage), não há workflow `deploy-homolog` separado — a estória cita `admin.homolog` por convenção legada.
