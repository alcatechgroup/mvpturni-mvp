<div data-testid="templates-catalogo">
    ...
    <h1 class="main-h">Templates</h1>
    <p class="main-d">Edite contratos e e-mails de notificação sem precisar de deploy. Cada edição cria uma nova versão; a ativa vale para os novos envios/cadastros.</p>

    <div class="panel">
        <div class="panel-h"><h3>Catálogo</h3></div>
        <table>
            <thead>
                <tr><th>Template</th><th>Categoria</th><th>Slug</th><th>Versão ativa</th><th>Ativada em</th><th class="right"><span class="sr-only">Ações</span></th></tr>
            </thead>
            <tbody>
                @foreach ($templates as $template)
                    ...
                    <tr wire:key="tpl-{{ $template->id }}" data-testid="templates-catalogo-item-{{ $template->slug }}">
                        ...
                        <td><span class="chip" data-testid="templates-catalogo-item-{{ $template->slug }}-categoria">{{ $template->categoria === 'email' ? 'E-mail' : 'Contrato' }}</span></td>
