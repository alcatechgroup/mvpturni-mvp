...
import 'package:turni_webapp/features/turnos/pin_checkin_screen.dart';
...
void main() {
  ...
  testWidgets('janela aberta: título, apoio e botão HABILITADO', (tester) async {
    final svc = _FakeDetalheService(() => TurnoDetalheSuccess(_turnoNaJanela()));
  ...
  testWidgets('antes da janela: botão desabilitado + microcopy com horário e minutos', (
    tester,
  ) async {
  ...
  testWidgets('depois da janela: botão desabilitado + microcopy de encerramento', (
    tester,
  ) async {
  ...
  testWidgets('aguardando_checkin: Gerar novo PIN + Cancelar PIN (pós-refresh §4.7)', (
    tester,
  ) async {
    final svc = _FakeDetalheService(
      () => TurnoDetalheSuccess(
    ...
    expect(
      find.text('Perdeu o PIN de vista? Gere um novo — o anterior deixa de valer.'),
      findsOneWidget,
    );
  ...
  testWidgets('CA-8 visual: contratante segue com o placeholder (sem botão de PIN)', (
    tester,
  ) async {
    final svc = _FakeDetalheService(
  ...
  testWidgets('toque em gerar: loading "Confirmando sua localização…" e push da tela do PIN', (
    tester,
  ) async {
  ...
  testWidgets('erro na geração: banner + retry refaz o gesto completo', (tester) async {
    var tentativa = 0;
    final pin = _FakePinService(
      onGerar: () async =>
          ++tentativa == 1 ? PinGeracaoErro() : PinGerado(pin: '7031', geofencing: _geoOk()),
    );
    final svc = _FakeDetalheService(() => TurnoDetalheSuccess(_turnoNaJanela()));
    await tester.pumpWidget(_app(svc, pin: pin));
    await tester.pumpAndSettle();
  ...
  testWidgets('fora da janela no servidor (422): recarrega o detalhe — sem banner de erro', (
    tester,
  ) async {
    final pin = _FakePinService(
  ...
  testWidgets('PIN em tipografia ≥64pt, instrução fixa do CA-5 e aviso de efemeridade', (
    tester,
  ) async {
    ...
    final codigo = tester.widget<Text>(find.byKey(const Key('pin-checkin-codigo')));
  ...
  testWidgets('nota de geofencing — ok: localização confirmada', (tester) async {
    await abrirTelaDoPin(tester, geo: _geoOk());
  ...
  testWidgets('nota de geofencing — fora do raio: distância + aviso ao contratante', (
    tester,
  ) async {
  ...
  testWidgets('timeline: checkin_solicitado com nota de geofencing e checkin_cancelado', (
    tester,
  ) async {
    final svc = _FakeDetalheService(
      () => TurnoDetalheSuccess(
