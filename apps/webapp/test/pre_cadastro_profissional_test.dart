...
class _FakeService extends CadastroService {
  ...
  Future<List<Funcao>> fetchFuncoes() async =>
      const [Funcao(id: 1, nome: 'Garçom / Garçonete')];
...
Future<void> _fillValid(WidgetTester tester) async {
  ...
  await tester.enterText(find.byKey(const Key('input-email')), 'diego@email.com');
  await tester.enterText(find.byKey(const Key('input-telefone')), '(11) 91234-5678');
  ...
  await tester.enterText(find.byKey(const Key('input-password')), 'SenhaForte10');
  await tester.enterText(find.byKey(const Key('input-password-confirm')), 'SenhaForte10');
...
void main() {
  ...
  testWidgets('submit vazio mostra erros de campo e bloqueia (CA-2/CA-5)', (tester) async {
    await _pump(tester, result: () => CadastroSuccess('ok'));
    ...
    expect(find.text('Selecione o tipo de cadastro: PF, MEI ou PJ.'), findsOneWidget);
    expect(find.text('Adicione uma foto.'), findsOneWidget);
    expect(
      find.text('É necessário aceitar os Termos de Uso e a Política de Privacidade.'),
      findsOneWidget,
    );
  ...
  testWidgets('caminho feliz envia e mostra a tela de recebido (CA-7)', (tester) async {
  ...
  testWidgets('e-mail já existente mostra banner genérico sem enumeração (CA-4)', (tester) async {
    await _pump(
      tester,
    ...
    expect(find.textContaining('Não foi possível concluir o cadastro'), findsOneWidget);
    expect(find.byKey(const Key('panel-cadastro-recebido')), findsNothing);
  });
