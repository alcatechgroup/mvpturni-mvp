...
Future<void> _pump(WidgetTester tester) async {
  await tester.pumpWidget(
    const MaterialApp(home: PerfilScreen()),
  );
...
void main() {
  ...
  testWidgets('(d) borda — contratante vê o rótulo de papel correto', (tester) async {
    AuthService().debugSetSession(_session(name: 'Marina Souza', role: 'contratante'));
    await _pump(tester);
    expect(find.text('Marina Souza'), findsOneWidget);
    expect(find.text('Contratante'), findsOneWidget);
  });

  testWidgets('(b) inválido — sessão sem nome não quebra (avatar fallback)', (tester) async {
  ...
  testWidgets('(a) feliz — alterna o tema escuro pelo switch e persiste no controller', (tester) async {
    AuthService().debugSetSession(_session());
    await _pump(tester);
  ...
  testWidgets('(a) feliz — expõe o sino de notificações e o botão Sair', (tester) async {
    AuthService().debugSetSession(_session());
    await _pump(tester);
